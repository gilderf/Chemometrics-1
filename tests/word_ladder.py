class Solution:
    ...
    def findLadders(self, beginWord, endWord, wordList):
        ...
        begin_lists = [[beginWord]]  # 当前状态
        end_lists = [[endWord]]
        while not is_terminal(begin_lists, end_lists):
            # 状态转移
            # filter or prone
            wordpool = _wordpool(begin_lists, end_lists)
            wordset = {word for word in wordset if word not in wordpool}

            # begin_move
            begin_lists = [_list + [word] for _list in begin_lists for word in wordset if D(_list[-1], word) == 1]
            wordpool = _wordpool(begin_lists, end_lists)
            wordset = {word for word in wordset if word not in wordpool}
            if is_terminal(begin_lists, end_lists): break
            end_lists = [[word] + _list for _list in end_lists for word in wordset if D(_list[0], word) == 1]

        if len(begin_lists) < 1 or len(end_lists) < 1: return []
        _ok = [blist + elist for blist in begin_lists for elist in end_lists if D(blist[-1], elist[0]) == 1]
        return _ok
...
def is_terminal(begin_states, end_states):
    """
    判断是否terminate, D(new_states[-1], endWord) ==1
    """
    if len(begin_states) < 1 or len(end_states) < 1: return True
    for bstate in begin_states:
        for estate in end_states:
            if D(bstate[-1], estate[0]) == 1:
                return True


def _wordpool(begins, ends):

    wordpool = []
    for l in begins+ends: wordpool += l
    wordpool = set(wordpool)
    return wordpool
...
if __name__ == "__main__":
    b = "sand"
    e = "acne"
    wl =  ["slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock", "fort", "heel", "send",
